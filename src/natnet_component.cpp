...
{
  NatnetComponent * node = static_cast<NatnetComponent *>(userData);
  auto stamp = node->now();
...
NatnetComponent::NatnetComponent(const rclcpp::NodeOptions & opts)
...
NatnetComponent::~NatnetComponent() {pImpl->client.Disconnect();}

bool NatnetComponent::connect()
{
...
bool NatnetComponent::get_data_description()
{
